...
A production-ready Node.js API with **Sliding Window** rate limiting, JWT authentication, **Redis-based state storage**, and progressive abuse prevention — written fully in **TypeScript** with strict type safety and compiled output.
...
```bash
# Development (ts-node-dev — runs TypeScript directly with auto-reload)
npm run dev
...
Extends Express `Request` with custom type-safe properties:
...
│   └── rateLimiter.ts        # Sliding window (Redis) + blocking logic
├── models/
│   ├── User.ts               # User schema
│   ├── AuditLog.ts           # Audit log schema (auto-expires 30d)
│   └── RateLimit.ts          # Rate limit model
├── routes/
│   ├── auth.ts               # /api/auth/*
│   └── api.ts                # /api/* (protected)
├── types/
...
| Rate Limiting    | Sliding Window (Redis)          | Sliding Window (Redis)           |